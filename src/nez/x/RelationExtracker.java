...
public class RelationExtracker {
	...
	void recieve(RNode t) {
		...
		appendBuffer(t);
		// System.out.println("index="+index + ", last+len=" + (lastExtracted +
		// buffer.length));
...
class Schema {
	...
	Schema union(Schema s2) {
		...
		for (int i = 0; i < m; i++) {
			/*
			 * The following is to merge two name lists in preserving partial
			 * orders ..
			 */
			if (i % n2 == 0) {
...
class RNode extends AbstractList<RNode> implements SourcePosition {
	...
	// @Override
	// public SyntaxTree newNode(Tag tag, Source source, long spos, long epos,
	// int size) {
	// return new RNode(this.tracker, tag == null ? this.tag : tag, source,
	// spos, epos, size);
	// }
