...
public class GrammarOptimizer {
	...
	class CheckerVisitor extends ExpressionDuplicationVisitor {
		TreeMap<String, Boolean> boolMap0;
		Conditions conds;
		...
		private void checkFirstVisitedProduction(String uname, Production p) {
			...
			this.requiredTypestate = this.isNonASTContext() ? Typestate.Unit : typeState(p);
		...
		public Expression visitNonTerminal(NonTerminal n, Object a) {
			...
			Typestate innerTypestate = this.isNonASTContext() ? Typestate.Unit : typeState(p);
		...
		public Expression visitLinkTree(Nez.LinkTree p, Object a) {
			...
			Typestate innerTypestate = this.isNonASTContext() ? Typestate.Unit : typeState(inner);
			if (innerTypestate != Typestate.Tree) {
				reportInserted(p, "{");
		...
		private Expression visitOptionalInner(Nez.Unary p) {
			Typestate innerTypestate = this.isNonASTContext() ? Typestate.Unit : typeState(p.get(0));
			if (innerTypestate == Typestate.Tree) {
				if (this.requiredTypestate == Typestate.TreeMutation) {
		...
		String uniqueName(String uname, Production p) {
			boolean isNonTree = typeState(p) == Typestate.Unit;
