...
public class PossibleAcceptance {
	...
	public static short acceptByteChar(int byteChar, int ch) {
		return (byteChar == ch) ? PossibleAcceptance.Accept
				: PossibleAcceptance.Reject;
	}

	public static short acceptByteMap(boolean[] byteMap, int ch) {
		return (byteMap[ch]) ? PossibleAcceptance.Accept
				: PossibleAcceptance.Reject;
	}

	public static short acceptAny(boolean binary, int ch) {
		if(binary) {
			return (ch == Source.BinaryEOF) ? PossibleAcceptance.Reject
					: PossibleAcceptance.Accept;
		}
		else {
			return (ch == Source.BinaryEOF || ch == 0) ? PossibleAcceptance.Reject
					: PossibleAcceptance.Accept;
		}
	}
	...
	public static short acceptOption(Expression e, int ch) {
		...
		return (r == PossibleAcceptance.Accept) ? r
				: PossibleAcceptance.Unconsumed;
	}

	public static short acceptAnd(And e, int ch) {
		short r = e.get(0).acceptByte(ch);
		return (r == PossibleAcceptance.Reject) ? r
				: PossibleAcceptance.Unconsumed;
	}
	...
	public static short acceptNot(Not e, int ch) {
		...
		if(inner instanceof ByteChar || inner instanceof ByteMap
				|| inner instanceof AnyChar) {
			return inner.acceptByte(ch) == PossibleAcceptance.Accept ? PossibleAcceptance.Reject
					: PossibleAcceptance.Unconsumed;
		}
