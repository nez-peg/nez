...
public class Sequence extends Expression {
	...
	/**
	 * @Override void optimizeImpl(int option) { if(UFlag.is(option,
	 *           Grammar.Optimization) && this.get(this.size() - 1) instanceof
	 *           AnyChar) { boolean byteMap[] = ByteMap.newMap(false);
	 *           if(isByteMap(option, byteMap)) { this.optimized =
	 *           GrammarFactory.newByteMap(s, byteMap); return; } // (!'ab'
	 *           !'ac' .) => (^[a]) / (!'ab' !'ac' .) if(UFlag.is(option,
	 *           Grammar.Prediction)) { ByteMap.clear(byteMap);
	 *           if(isPredictedNotByteMap(0, this.size() - 1, byteMap, option))
	 *           { this.optimized = GrammarFactory.newChoice(s,
	 *           GrammarFactory.newByteMap(s, byteMap), this); return; } } }
	 *           if(UFlag.is(option, Grammar.DFA) &&
	 *           needsReplaceOperation(option)) { this.optimized =
	 *           operationReplacedSequence(option);
	 *           //System.out.println("replaced: " + this + "\n => " +
	 *           this.optimized); } else { this.optimized = this; } }
	 * 
	 *           private boolean needsReplaceOperation(int option) { for(int i =
	 *           1; i < this.size(); i++) { Expression p =
	 *           this.get(i-1).optimize(option); Expression e =
	 *           this.get(i).optimize(option); if(Expression.isByteConsumed(e))
	 *           { if(Expression.isPositionIndependentOperation(p) || p
	 *           instanceof New || p instanceof Capture) { return true; } } }
	 *           return false; }
	 * 
	 *           private Expression operationReplacedSequence(int option) {
	 *           UList<Expression> l = this.toList(); for(int i = 1; i <
	 *           l.size(); i++) { Expression p =
	 *           l.ArrayValues[i-1].optimize(option); Expression e =
	 *           l.ArrayValues[i].optimize(option);
	 *           if(Expression.isByteConsumed(e)) {
	 *           if(Expression.isPositionIndependentOperation(p)) {
	 *           l.ArrayValues[i-1] = e; l.ArrayValues[i] = p; continue; } if(p
	 *           instanceof New) { New n = (New)p; l.ArrayValues[i-1] = e;
	 *           if(n.isInterned()) { l.ArrayValues[i] =
	 *           GrammarFactory.newNew(n.s, n.lefted, n.shift - 1); } else {
	 *           n.shift -= 1; l.ArrayValues[i] = n; } continue; } if(p
	 *           instanceof Capture) { Capture n = (Capture)p;
	 *           l.ArrayValues[i-1] = e; if(n.isInterned()) { l.ArrayValues[i] =
	 *           GrammarFactory.newCapture(n.s, n.shift - 1); } else { n.shift
	 *           -= 1; l.ArrayValues[i] = n; } continue; } } } return
	 *           GrammarFactory.newSequence(s, l); }
	 * 
	 * 
	 *           boolean isByteMap(int option, boolean[] byteMap) { for(int i =
	 *           0; i < this.size() - 1; i++) { Expression p =
	 *           this.get(i).optimize(option); if(p instanceof Not) { p =
	 *           p.get(i).optimize(option); if(p instanceof ByteChar) {
	 *           byteMap[((ByteChar) p).byteChar] = true; continue; } if(p
	 *           instanceof ByteMap) { ByteMap.appendBitMap(byteMap, ((ByteMap)
	 *           p).byteMap); continue; } } return false; }
	 *           ByteMap.reverse(byteMap, option); return true; }
	 * 
	 * 
	 *           boolean isPredictedNotByteMap(int start, int end, boolean[]
	 *           byteMap, int option) { for(int i = start; i < end; i++) {
	 *           Expression p = this.get(i); //.optimize(option); if(p
	 *           instanceof Not) { p = p.get(i).optimize(option); predictByte(p,
	 *           byteMap, option); continue; } return false; }
	 *           ByteMap.reverse(byteMap, option); return true; }
	 * 
	 *           void predictByte(Expression e, boolean[] byteMap, int option) {
	 *           for(int c = 0; c < 256; c++) { if(e.acceptByte(c, option) !=
	 *           Acceptance.Reject) { byteMap[c] = true; } } }
	 **/
