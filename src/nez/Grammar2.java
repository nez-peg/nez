...
public class Grammar2 {
	...
	Grammar2(Production start, int option) {
		this.start = start;
...
//		}
	...
	public final void record(Recorder rec) {
		if(rec != null) {
			this.enable(Grammar2.Profiling);
			this.compile();
	...
	public final static String stringfyOption(int option, String delim) {
		...
		if(UFlag.is(option, Grammar2.ClassicMode)) {
			sb.append(delim);
			sb.append("classic");
		}
		...
		if(UFlag.is(option, Grammar2.PackratParsing)) {
			sb.append(delim);
			sb.append("memo");
		}
		if(UFlag.is(option, Grammar2.Optimization)) {
			sb.append(delim);
			sb.append("opt.");
		}
		...
		if(UFlag.is(option, Grammar2.CommonPrefix)) {
			sb.append(delim);
			sb.append("com.");
		}
		if(UFlag.is(option, Grammar2.Inlining)) {
			sb.append(delim);
			sb.append("inline");
		}
		...
		if(UFlag.is(option, Grammar2.Profiling)) {
			sb.append(delim);
			sb.append("prof");
		}
